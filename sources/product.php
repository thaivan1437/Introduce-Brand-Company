...
if ($id != '') {
    ...
    $d->reset();
    $sql_sanphamkhac = "select id,thumb,ten_$lang as ten, mota_$lang as mota, photo,tenkhongdau,gia,masp,spbc,giakm,rate, luot_rate,noibat, luotxem, id_list, spkm, spbc, type, ngaytao from #_product where type='".$type."' and hienthi=1 and id !='" . $row_detail["id"] . "' order by stt,ngaytao desc limit 0,12";
...
$d->reset();
$sql="select ten_$lang as ten, tenkhongdau, id,photo,noidung_$lang as noidung, ngaytao,ngaysua from #_product_list where type='san-pham' and com=1 and hienthi=1 order by stt, id desc";
$d->query($sql);
$rs_pr_list=$d->result_array();


    $tongsanpham = count($rs_pr_list);
    $curPage = isset($_GET['p']) ? $_GET['p'] : 1;
    $url = getCurrentPageURL();
    $maxR = 15;
    $maxP = 5;
    $paging = paging_home($rs_pr_list, $url, $curPage, $maxR, $maxP);
    $rs_pr_list = $paging['source'];
}
?>