...
function save_item() {
    ...
    if ($id) { 
		...
		$data['id_item'] = (int)($_POST["id_parent"][$count-1]);
			...
			if (isset($_FILES['files'])) {
	            ...
	            for ($i = 0; $i < $fileCount; $i++) { 
					...
					if(empty($file_info)) // No Image?
					{
						...
						if(move_uploaded_file($myFile["tmp_name"][$i], _upload_product."/".$file_name."_".changeTitle($name).'.' . $ext)){	
...
function save_list() {
	...
	if ($id) {
		...
		$count=count($_POST["id_parent"]);
		$data['id_parent'] = (int)($_POST["id_parent"][$count-1]);
        $data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
        $data['ngaysua'] = time();
