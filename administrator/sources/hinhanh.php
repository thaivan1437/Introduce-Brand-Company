...
function get_items() {
    ...
    $sql = "select * from #_hinhanh where type='".$type."' ";
    ...
    $url = "default.php?com=hinhanh&act=man&type=".$type."";
    $maxR = 20;
    $maxP = 4;
...
function get_item() {
    global $d,$type, $item, $ds_photo;
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "default.php?com=hinhanh&act=man&type=".$type."");

    $sql = "select * from #_hinhanh where id='" . $id . "' and type='".$type."'";
    $d->query($sql);
    if ($d->num_rows() == 0)
...
function save_item() {
    ...
    if ($id) {
        ...
        if ($d->update($data)){
			...
			redirect("default.php?com=hinhanh&act=man&type=".$type."");
		}
            
        else
            transfer("Cập nhật dữ liệu bị lỗi", "default.php?com=hinhanh&act=man&type=".$type."");
    }else {
        ...
        if ($d->insert($data)){
			...
			if (isset($_FILES['files'])) {
	            ...
	            for ($i = 0; $i < $fileCount; $i++) { 
					...
					$name = basename($myFile["name"][$i], '.' . $ext);
					if(empty($file_info)) // No Image?
					{
						$error .= "Không đúng định dạng file hình";
					}else{//A Image
						if(move_uploaded_file($myFile["tmp_name"][$i], _upload_hinhanh."/".$file_name."_".changeTitle($name).'.' . $ext)){								
							$data1['stt'] = (int)$_POST['stthinh'][$i];
							$data1['photo'] = $file_name."_".changeTitle($name).'.' . $ext;	
            ...
            transfer("Lưu dữ liệu bị lỗi", "default.php?com=hinhanh&act=man&type=".$type."");
    }
}
