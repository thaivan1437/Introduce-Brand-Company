...
function dangky_tv() {
    ...
    $thongbao = ' <div class="error_mess">' . $error_matkhau . $error_golaimatkhau . $error_hoten . $error_username.'</div>';
    ...
    if ($coloi == FALSE) {
		...
		$mailid = strtoupper(ChuoiNgauNhien(6));
        ...
        if (!$mail->Send()) {
			...
			$sql = "INSERT INTO  table_member (username,password,old_password,name, created)
				  VALUES ('$username','$matkhau','$matkhau_old','$ten','$ngaydangky')";
    ...
    if ($d->num_rows() == 1) {
        ...
        if ($row['hienthi'] ==1) {
            if ($row['password'] == md5($password)) {
                ...
                $_SESSION['login_web']['ten'] = $row['name'];
				$rs["id"]=1;
                ...
                $rs["id"]=0;
				$rs["thongbao"]="<span style='color:#428bca'>Sai tên đăng nhập. Vui lòng kiểm tra lại</span><br><a href='quen-mat-khau.html'>Quên mật khẩu</a>";
			}
        } else{
            $rs["id"]=0;
